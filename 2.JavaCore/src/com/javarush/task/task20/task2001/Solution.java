...
public class Solution {
    ...
    public static class Human {
        ...
        public void load(InputStream inputStream) throws Exception {
            ...
            String newStr[] = new String(buffer).split("\\|");
            System.out.println();
            name = newStr[0];

            assets = new ArrayList<>();

            for (int i = 1; i < newStr.length - 1; i += 2) {
                assets.add(new Asset(newStr[i]));
                assets.get((i - 1) / 2).setPrice(Double.valueOf(newStr[i+1]));
            }
        }
    }
}
